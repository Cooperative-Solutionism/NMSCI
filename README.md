...
`source-code-zip-hash` 不应手工维护。它会在 Maven 打包阶段由 `CalcSourceCodeZipHash` 计算后写入构建产物中的 `application.properties`。
...
2. 根据当前 `block-version` 生成 `source_code_v{block-version}.zip`。
3. 计算源码包 SHA-256。
4. 将哈希写入构建产物中的 `source-code-zip-hash`。
...
unzip -p target/nmsci-1.0.0.jar BOOT-INF/classes/application.properties \
  | grep -E 'block-version|source-code-zip-hash'
```
...
当协议或实现发生需要上链声明的变化时，应递增 `block-version` 并重新部署。
...
- 每次协议或实现升级都应递增 `block-version`，不要复用旧版本号。
