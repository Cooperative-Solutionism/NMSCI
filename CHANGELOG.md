...
- **Flyway 数据库迁移**，替换手写双轨 schema/patches；基线对齐线上真实模式，安全升级既有 v1.0.0 库（见 [docs/DB-BASELINE.md](./docs/DB-BASELINE.md)）。
...
### 迁移与升级
- 数据库迁移 **V1–V5**（V1 基线快照 / V2 约束 + 索引 / V3 硬化 / V4 公钥轮换 / V5 交易时间戳索引）。既有生产库升级前**必须**执行数据预检（V2/V3 追加唯一/检查/外键约束，存量违例将导致启动失败）；V4 放宽约束、V5 仅新增索引，二者均无需预检。详见 [docs/DB-BASELINE.md](./docs/DB-BASELINE.md)。
