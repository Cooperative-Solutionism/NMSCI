 ...
 * @param maxSupportedBlockVersion  本验证器支持核验的最高区块版本；版本在 [1, 此值] 内的区块按当前规则核验，超出者判为「过新、需升级验证器」。默认取 {@link BlockConstants#MAX_SUPPORTED_BLOCK_VERSION}。
 */
public record VerifierOptions(
    ...
    public static final class Builder {
        ...
        private int maxSupportedBlockVersion = BlockConstants.MAX_SUPPORTED_BLOCK_VERSION;
        ...
        public Builder maxSupportedBlockVersion(int value) {
            this.maxSupportedBlockVersion = value;
            return this;
        }

        public VerifierOptions build() {
            return new VerifierOptions(expectedCentralPubkey, expectedSourceHashHex, startingPreviousHash,
                    includeStatefulReplay, maxSupportedBlockVersion);
        }
    }
}
